...
Certains sorts, tels *[[10.1-sorts|projectile magique]]* et *[soins](https://www.aidedd.org/dnd/sorts.php?vf=soins)*, ont des effets plus puissants lorsque le sort est lancé à un niveau supérieur. Ceci est indiqué dans la description du sort.
...
La cible d'un sort doit être dans la portée du sort. Par exemple, pour un sort comme [[10.1-sorts|projectile magique]], la cible est une créature. Pour un sort comme [[10.1-sorts|boule de feu]], la cible est un point dans l'espace ou la boule de feu explose. La plupart des sorts ont des portées exprimées en mètres. Certains sorts peuvent ne cibler qu'une créature (vous y compris) que vous devez toucher. D'autres sorts, comme le sort [[10.1-sorts|bouclier]], n'affectent que vous. Ces sorts indiquent que la portée est personnelle.
...
La plupart des sorts exigent de psalmodier des mots mystiques. Les mots en eux-mêmes ne sont pas la source de la puissance du sort ; c'est plutôt la combinaison particulière de sons avec un terrain et une résonance spécifique qui fixe les toiles de la magie en mouvement. Ainsi, un personnage qui est bâillonné ou est dans une zone de silence, comme celle créée par le sort [[10.1-sorts|silence]], ne peut pas lancer un sort à composante verbale.
...
Par exemple, si deux clercs lancent [[10.1-sorts|bénédiction]] sur la même cible, le personnage a le bénéfice seulement d'un des deux sorts ; il ne peut pas lancer deux fois les dés de bonus.
